defmodule SlackNotification do
  ...
  def send_notification(message) do
    ...
    missing_envs =
      if slack_webhook_url == false, do: missing_envs ++ ["slack_webhook_url"], else: missing_envs

    slack_channel = Application.get_env(:mllparty, :slack_channel, false)

    missing_envs =
      if slack_channel == false, do: missing_envs ++ ["slack_channel"], else: missing_envs
    ...
    if length(missing_envs) == 0 do
      ...
      Logger.warn(
        "Environment variable is not set: #{Enum.join(missing_envs, ", ")}. Slack notification will not be sent (Message: #{message})"
      )
    end
  end
